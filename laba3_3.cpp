...
pair<int, int> getRationalSum(int a, int b) {
    ...
    const int MAX_DENOMINATOR = 1000000; // для преобразования в дробь макс. знаменатель

    double sum = 0.0;//инициализация суммы ряда

    for (int n = 1; ; ++n) {
        double term = pow(n, a) / pow(b, n);
        if (term < EPS) break;//// Если член меньше точности - прекращаем суммирование
        sum += term;
    }

    // Преобразуем в несократимую дробь с макс. знаменателем
